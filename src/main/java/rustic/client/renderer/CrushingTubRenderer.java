...
public class CrushingTubRenderer extends TileEntitySpecialRenderer<TileEntityCrushingTub> {
	...
	public void renderTileEntityAt(TileEntityCrushingTub te, double x, double y, double z, float partialTicks,
		...
		if (fluid != null){
            ...
            buffer.pos(x+0.0625, y+0.0625+0.5*((float)amount/(float)capacity), z+0.06255).tex(minU, minV).lightmap(lightx,lighty).color(red,green,blue,alpha).endVertex();
		...
		if (itemStackHandler.getSlots() > 0 && !itemStackHandler.getStackInSlot(0).isEmpty() && tank.getWorld() != null) {
			...
			for (int i = 0; i < itemCount; i ++){
	            ...
	            GlStateManager.enableLighting();
				GL11.glTranslated(x, y+0.0625+(i*0.0625), z);
